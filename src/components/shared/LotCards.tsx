...
export default function LotCards({
  ...
  const lotActions = (lot: LotWithSummary, size: "sm" | "compact") => (
    <div
      className={cn(
        "flex items-center",
        size === "compact" ? "gap-1" : "gap-1 sm:gap-2"
      )}
    >
      <Button
        variant="ghost"
        ...
        className={cn(
          "hover:bg-muted p-0",
          size === "compact" ? "h-7 w-7" : "h-6 w-6 sm:h-7 sm:w-7"
        )}
      ...
      <Button
        ...
        className={cn(
          "hover:bg-destructive/10 hover:text-destructive p-0",
          size === "compact" ? "h-7 w-7" : "h-6 w-6 sm:h-7 sm:w-7"
        )}
        ...
        >
          <CardContent className="w-full p-0">
            ...
            <div className="relative w-full">
              <Link
                href={`/income/${lot.id}`}
                className="hover:bg-muted/30 block transition-colors duration-200"
              >
              ...
              <div
                className={`bg-muted/20 border-muted/30 flex items-center border-t px-2 py-1 sm:px-4 sm:py-1.5 ${isAdmin ? "justify-between lg:justify-end" : "justify-start lg:hidden"}`}
              >

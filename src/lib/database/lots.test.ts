...
// to return maintenanceActiveFrom as a raw Date, which shifted a day once
...
describe.each(["UTC", "America/Bogota"])("toLot (TZ=%s)", (tz) => {
  ...
  it("converts maintenanceActiveFrom to a YYYY-MM-DD string that round-trips what was written", () => {
    vi.stubEnv("TZ", tz);
    // parseLocalDate("2026-01-01") is exactly what createLot/updateLot
    // stores server-side for this input.
    const raw = makePrismaLot({
      maintenanceActiveFrom: parseLocalDate("2026-01-01"),
    });
    expect(toLot(raw).maintenanceActiveFrom).toBe("2026-01-01");
  });

  it("passes through a null maintenanceActiveFrom unchanged", () => {
    vi.stubEnv("TZ", tz);
    const raw = makePrismaLot({ maintenanceActiveFrom: null });
    expect(toLot(raw).maintenanceActiveFrom).toBeNull();
  });
});

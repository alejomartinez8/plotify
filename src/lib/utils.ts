...
import { SimpleLotBalance, LotDebtDetail, LotPaymentStatus } from "@/types/quotas.types";
...
export function calculateLotDebtDetail(
  ...
  const worksDebt = Math.max(0, (worksQuotas + lotWithContributions.initialWorksDebt) - worksContributions);
  
  const totalQuotas = maintenanceQuotas + worksQuotas + lotWithContributions.initialWorksDebt;
...
/**
 * Returns localized text for payment status display
 * @param status - The payment status of the lot
 * @returns Localized status text in Spanish
 */
export function getStatusText(status: LotPaymentStatus): string {
  switch (status) {
    case "current":
      return "Al día";
    case "overdue":

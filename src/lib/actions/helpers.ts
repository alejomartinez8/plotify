import { requireAdmin, requireTreasurer } from "@/lib/auth";
...
/**
 * Helper to check treasurer access and handle errors consistently
 * Returns an error state if access is denied, or null if authorized
 */
export async function checkTreasurerAccess<T extends { success?: boolean; message?: string | null }>(
  actionTimer: { end: () => void },
  errorMessage: string
): Promise<T | null> {
  try {
